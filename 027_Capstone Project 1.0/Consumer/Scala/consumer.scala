...
{	
	def main(args: Array[String]): Unit =
	{
		...
		val ss = SparkSession.builder.master("local").appName("NYTimes").config(conf).getOrCreate()
		...
		val mappedlines = dstream.foreachRDD{ rdd =>
      		rdd.foreach{record =>
			     ...
			     {
				df2.write.jdbc(s"jdbc:mysql://localhost:3306/bigData", "NY_Times_Table", prop)
			     }
			   }
